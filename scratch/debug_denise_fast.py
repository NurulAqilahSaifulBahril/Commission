...
def main():
    ...
    for inv_num, inv in sorted(invoices.items()):
        print(f"Invoice: {inv['invoice_number']} | Cust: {inv['customer_name']} | Date: {inv['invoice_date'][:10]} | Total: RM {float(inv['total_amount']):,.2f} | Full Pay Date (DB): {inv['full_payment_date']}")
        pay_sum = 0.0
        for p in inv['payments']:
            print(f"  Payment: ID={p['id']}, Amt=RM {float(p['amount']):,.2f}, Date={p['date'][:10]}")
            pay_sum += float(p['amount'])
        print(f"  Total Payments sum: RM {pay_sum:,.2f} ({(pay_sum / float(inv['total_amount']) * 100):.1f}%)")
        print("-" * 80)
